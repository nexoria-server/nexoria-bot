...
ANNOUNCEMENT_PANEL_CHANNEL_ID = settings.ANNOUNCEMENT_PANEL_CHANNEL_ID
...
REQUIRE_ADMINISTRATOR = settings.ANNOUNCEMENT_REQUIRE_ADMIN
ALLOWED_ROLE_IDS: set[int] = set(settings.ANNOUNCEMENT_ALLOWED_ROLE_IDS)

LOCK_PANEL_CHANNEL = bool(ANNOUNCEMENT_PANEL_CHANNEL_ID)

# Kanal, in dem private Claim-Tickets für Giveaway-Gewinner erstellt werden.
GIVEAWAY_CLAIM_CHANNEL_ID = settings.GIVEAWAY_CLAIM_CHANNEL_ID
...
class Announcements(commands.Cog):
    ...
    async def cog_load(self):
        ...
        asyncio.create_task(self.initialize_panel())
        asyncio.create_task(self.restore_giveaway_views())
        asyncio.create_task(self.restore_claim_views())
    ...
    async def ensure_panel(self):
        if not ANNOUNCEMENT_PANEL_CHANNEL_ID:
            log.info(
                "[ANNOUNCE] Kein ANNOUNCEMENT_PANEL_CHANNEL_ID konfiguriert; "
                "automatisches Panel wird übersprungen."
            )
            return
